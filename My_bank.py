...
def create_user(users):
    IBAN = input("Enter the IBAN (numbers only): ")
    user = filter_user(IBAN, users)

    if user:
        print("\n User with this IBAN already exists! ")
        return
...
# Function to filter a user by IBAN
def filter_user(IBAN, users):
    filtered_users = [user for user in users if user["IBAN"] == IBAN]
    return filtered_users[0] if filtered_users else None

# Function to create a new account
def create_account(branch, account_number, users):
    IBAN = input("Enter the user's IBAN (numbers only): ")
    user = filter_user(IBAN, users)
